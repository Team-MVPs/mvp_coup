	...
	return (<div>
				{Object.keys(all_moves).map(move => (
					<div style ={{paddingBottom: "1em", paddingTop: "1em"}}>
						<Button type="button" className="btn btn-lg btn-light" style = {{width:"20em"}} onClick={all_moves[move](props.roomName, props.currentTurn, props.playerName)}>{move}</Button>
					</div>
					))}
			</div>);
}
